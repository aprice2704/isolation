...
class MinimaxPlayer(IsolationPlayer):
 ...
 #       super().__init__()
    ...
    def minimax(self, game, depth):
        ...
        **********************************************************************
        ...
        -----
            ...
            (2) If you use any helper fusnctions (e.g., as shown in the AIMA
   ...
   #     print ("Max>", myvalues)
    ...
    def max_value(self, game, mydepth, maxdepth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        if mydepth >= maxdepth:
        ...
        if len(legals) == 0:
            return -1
        mvs = [self.min_value(game.forecast_move(move), mydepth + 1, maxdepth) for move in legals]
        if len(mvs) == 0:
            return -1
  #      print("MVS-max>", mvs)
        return max(mvs)

    def min_value(self, game, mydepth, maxdepth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        if mydepth >= maxdepth:
            return self.score(game, self)
        ...
        if len(legals) == 0:
            return 999
        mvs = [self.max_value(game.forecast_move(move), mydepth + 1, maxdepth) for move in legals]
        if len(mvs) == 0:
            return 999
  #      print ("MVS-min>", mvs)
        return min(mvs)
        ...
        **********************************************************************
        ...
        try:
            ...
            for depth in range(1, game.width*game.height):
                ...
                if result != (-1,-1):
                    sc = self.score( self.game.forecast_move(result), self.player)
                    if sc > best_score:
                        best_move = result
                        best_score = sc
                    return result
